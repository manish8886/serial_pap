...
static inline void DOWNLINK_SEND_ROTORCRAFT_CAM_STICK(struct DownlinkTransport *tp, uint8_t *_ac_id, int8_t *_tilt, int8_t *_pan){ 
	if (tp->CheckFreeSpace(tp->impl, tp->SizeOf(tp->impl, 0+1+1+1))) {
	  tp->CountBytes(tp->impl, tp->SizeOf(tp->impl, 0+1+1+1)); 
	  tp->StartMessage(tp->impl, "ROTORCRAFT_CAM_STICK", DL_ROTORCRAFT_CAM_STICK, 0+1+1+1); 
	  tp->PutBytes(tp->impl, DL_TYPE_UINT8, 1, (void *) _ac_id); 
...
static inline void DOWNLINK_SEND_HITL_GPS_COMMON(struct DownlinkTransport *tp, uint8_t *_gps_id, uint8_t *_ac_id, double *_lat, double *_lon, double *_alt, double *_course, double *_gspeed, double *_climb, double *_time){ 
	if (tp->CheckFreeSpace(tp->impl, tp->SizeOf(tp->impl, 0+1+1+8+8+8+8+8+8+8))) {
	  tp->CountBytes(tp->impl, tp->SizeOf(tp->impl, 0+1+1+8+8+8+8+8+8+8)); 
	  tp->StartMessage(tp->impl, "HITL_GPS_COMMON", DL_HITL_GPS_COMMON, 0+1+1+8+8+8+8+8+8+8); 
	  tp->PutBytes(tp->impl, DL_TYPE_UINT8, 1, (void *) _gps_id); 
	  tp->PutBytes(tp->impl, DL_TYPE_UINT8, 1, (void *) _ac_id); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _lat); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _lon); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _alt); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _course); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _gspeed); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _climb); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _time); 
	  tp->EndMessage(tp->impl); 
	} else 
	  tp->Overrun(tp->impl); 
}

static inline void DOWNLINK_SEND_HITL_IR_AHRS(struct DownlinkTransport *tp, uint8_t *_ir_id, uint8_t *_ac_id, double *_roll, double *_pitch, double *_yaw, double *_p, double *_q, double *_r){ 
	if (tp->CheckFreeSpace(tp->impl, tp->SizeOf(tp->impl, 0+1+1+8+8+8+8+8+8))) {
	  tp->CountBytes(tp->impl, tp->SizeOf(tp->impl, 0+1+1+8+8+8+8+8+8)); 
	  tp->StartMessage(tp->impl, "HITL_IR_AHRS", DL_HITL_IR_AHRS, 0+1+1+8+8+8+8+8+8); 
	  tp->PutBytes(tp->impl, DL_TYPE_UINT8, 1, (void *) _ir_id); 
	  tp->PutBytes(tp->impl, DL_TYPE_UINT8, 1, (void *) _ac_id); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _roll); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _pitch); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _yaw); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _p); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _q); 
	  tp->PutBytes(tp->impl, DL_TYPE_DOUBLE, 8, (void *) _r); 
	  tp->EndMessage(tp->impl); 
	} else 
	  tp->Overrun(tp->impl); 
}
